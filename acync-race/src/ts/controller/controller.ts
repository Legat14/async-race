...
export class Controller {
  private millisecsInSec = 1000;

  private frameFreq = 60;

  private frameDuration = this.millisecsInSec / this.frameFreq;

  private carWidth = 90;

  private timersArr: NodeJS.Timer[] = [];
  ...
  create100RandomCarsEvent(): void {
    const create100RandomCarsBtn: HTMLButtonElement | null = document.querySelector('.header-control-panel__create-100-random-cars-btn');
  ...
  deleteAllCarsEvent(): void {
    const deleteAllCarsBtn: HTMLButtonElement | null = document.querySelector('.header-control-panel__delete-all-cars-btn');
  ...
  createCarEvent(): void {
    const createCarsBtn: HTMLButtonElement | null = document.querySelector('.create-car-div__create-car-btn');
    const carNameInput: HTMLInputElement | null = document.querySelector('.create-car-div__car-name-input');
    const carColorInput: HTMLInputElement | null = document.querySelector('.create-car-div__car-color-input');
  ...
  private async carGo(car: HTMLDivElement): Promise<void> {
    if (car.dataset.carId) {
      ...
      const currentPosition: number = +((car.style.left.replace)('px', ''));
  ...
  private async setCarSpeed(
    ...
    if (car.dataset.carId) {
      this.timersArr[+car.dataset.carId] = setInterval((): void => {
        const stopBtn:
        HTMLButtonElement | null = document.querySelector(`.track-car-controls__car-stop-btn
        [data-car-id="${car.dataset.carId}"]`);
        if (stopBtn) {
          if (stopBtn.disabled) {
  ...
  private async getTrackLength(): Promise<number> {
  ...
  private async getEngineData(car: HTMLDivElement): Promise<string> {
    let status = 'drive';
    if (car.dataset.carId) {
      const engineDriveData:
      [number, CarEngineDrive] = await dataModel.getEngine(
        +car.dataset.carId,
        'drive',
      ) as [number, CarEngineDrive];
  ...
  private async carStop(car: HTMLDivElement, status: string) {
    if (status === 'stop') {
      if (car.dataset.carId) {
  ...
  private async carBroke(car: HTMLDivElement, status: string) {
    if (status === 'stop') {
      if (car.dataset.carId) {
        this.timersArr.forEach((timer, index): void => {
          if (car.dataset.carId) {
  ...
  private async carReturn(car: HTMLDivElement) {
    car.style.left = '0';
    const goBtn:
    HTMLButtonElement | null = document.querySelector(`.track-car-controls__car-go-btn
    [data-car-id="${car.dataset.carId}"]`);
    if (goBtn) {
  ...
  private toggleButton(button: HTMLButtonElement) {
    if (button.disabled) {
  ...
  async startRaceEvent(): Promise<void> {
    const raceBtn:
    HTMLButtonElement | null = document.querySelector('.header-race-panel__race-btn');
    raceBtn?.addEventListener('click', async (): Promise<void> => {
      this.noWinner = true;
      this.start = this.startTimer();
      const resetRaceBtn:
      HTMLButtonElement | null = document.querySelector('.header-race-panel__reset-race-btn');
      this.toggleButton(raceBtn);
  ...
  private async showWinner(start: number, stop: number, car: HTMLDivElement) {
    if (this.noWinner) {
  ...
  private async addEventsToButtons() {
